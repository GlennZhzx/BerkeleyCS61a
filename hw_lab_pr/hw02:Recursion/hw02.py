...
def num_eights(x):
    ...
    reminder = x % 10
    if diss == 0:
        return sum([reminder == 8])
    else:
        return num_eights(diss) + sum([reminder == 8])
...
def pingpong(n):
    ...
    if n == 2:
        return 2
    if n > 2:
        if pingpong(n - 1) % 8 == 0 or num_eights(pingpong(n - 1)) != 0:
            return pingpong(n - 2)
        else:
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        diss = n // 10
        reminder = n % 10
        reminderTemp = diss % 10
        diff = reminder - reminderTemp - 1 
        if diff > 0:
            return diff + missing_digits(diss)
        else:
            return missing_digits(diss)
