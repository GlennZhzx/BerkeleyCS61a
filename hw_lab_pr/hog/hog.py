...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            return announce_highest(0, score0, max(diff, running_high))
        else:
            diff = score1 - last_score
            if diff > running_high:
                print(diff, "point(s)! The most yet for player 1")
            return announce_highest(1, score1, max(diff, running_high))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num_rolls in range(1, 11):
        pointAverage = make_averaged(roll_dice, trials_count)(num_rolls, dice)
        if pointAverage > highestResult:
            highestResult = pointAverage
            number = num_rolls
    return number
   # END PROBLEM 9
